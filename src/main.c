...
{   
    ...
    client_init(&server_client,tcp_ip_server.socketd);
    server_add_client(&tcp_ip_server,&server_client);
    ...
    while(1){
        ...
        list_for_each_entry(&tcp_ip_server.clients, client, node) { //
            ...
            else if (server_is_pollin_revent(client)){

                server_manage_pollin_revent(client);
            }   
            else{
